 ...
 * Título: T5 - Implementação da Árvore B
 ...
 * O trabalho apresenta X funcionalidades distinas que são 
...
int main(void){
    ...
    NoArvoreB     noBT, noBT2;
    HeaderArvoreB cabecalhoBT, hbt, hbt2;
    ...
    int     qtdRegistros, qtdRemocoes, qtdInsercoes, qtdAtualizacoes, qtdNiveis;
    char    campoBusca[64];
    char    tagCampoBusca;
    char    valorBusca[164];
    ...
    switch(opcao){
        ...
        case 15:
            ...
            if(gvl_criarBTree(&bin, &cabecalho, &binIndex)){
                printf("Falha no processamento do arquivo.\n");
                break;
            }
        ...
        case 16:
            ...
            cabecalho      = gvl_carregarCabecalho(&bin);
            cabecalhoBT    = gvl_carregarCabecalhoArvoreB(&binIndex);

            if(cabecalho.status != '1' || cabecalhoBT.status != '1'){
                printf("Falha no processamento do arquivo.\n");
                break;
            } 

            if(sscanf(valorBusca, "%d", &regAux.idServidor) != 1){
                printf("Falha no processamento do arquivo.\n");
                break;
            }


            qtdNiveis = gvl_buscarRegistros3(&bin, &cabecalho, &binIndex, &cabecalhoBT, regAux.idServidor);
        ...
        // Validar Nó da Árvore B
        case 21:

            gvl_abrirArquivo(&bin,  "binarios/binario-2.btree", "r+");
            gvl_abrirArquivo(&bin2, "binariosBackup/binario-2.btree", "r+");

            hbt  = gvl_carregarCabecalhoArvoreB(&bin);
            hbt2 = gvl_carregarCabecalhoArvoreB(&bin2);

            for(int i = 0; i < 20; i++){
                int exibir = 0;
                
                gvl_lerNoArvoreB(&bin, &hbt, &noBT, i);
                gvl_lerNoArvoreB(&bin2, &hbt2, &noBT2, i);

                for(int i = 0; i < BTREE_ORDEM-1; i++){
                    if(noBT.keys[i].val != noBT2.keys[i].val || noBT.keys[i].ptr != noBT2.keys[i].ptr)
                        exibir = 1;
                }

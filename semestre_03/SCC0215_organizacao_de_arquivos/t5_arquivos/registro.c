...
/** Funções para manipulação da lista de indices
 */
    int gvl_criarVetorIndices(Arquivo* bin, Header* h, Index** indices, HeaderIndex* hi){
        if(bin == NULL || bin->fp == NULL || h == NULL || indices == NULL || hi == NULL)
            return 1;

        // Limpando o vetor de indices caso ele já tenha sido estanciado
        gvl_destruirIndices(indices); 
        hi->nroRegistros    = 0;
        hi->status          = '1';

        // Percorrendo os registros do arquivo e adicionando no vetorIndices
        Registro    reg;
        Index       i;
        int         qtdRegistros = 0;
        long        byteOffset;
        ...
        do{ 
            byteOffset  = bin->posicaoPtr;
            reg         = gvl_carregarRegistro(bin);
            
            if(reg.removido == GVL_LIXO_MEM){
                gvl_destruirRegistro(&reg);
                break;
            }

            if(reg.removido == '*' || reg.nomeServidor == NULL){
                gvl_destruirRegistro(&reg);
                continue;
            }

            // Limpa e copia a byteOffset e a chave de busca
            memset(&i, GVL_LIXO_MEM, sizeof(Index));
            i.byteOffset = byteOffset;
            strcpy(i.chaveBusca, reg.nomeServidor);
           
            // Inserindo o indice no vetor de indices           
            gvl_inserirIndice(indices, hi, i);
        ...
        gvl_ordenarIndices(hi, indices);

        return 0;
    }
    
    int gvl_buscarRegistros2(Arquivo* bin, Header* h, Index** indices, HeaderIndex* hi, char* valor){
        if(bin == NULL || bin->fp == NULL || indices == NULL || hi == NULL || valor == NULL)
            return 0;

        Registro aux;
        int qtdRegistros = 0, posIndex;
        
        // Buscando o valor no vetor de indices
        posIndex = gvl_buscarIndice(hi, indices, valor);

        // Caso a busca não tenha encontrado ninguem
        if(posIndex == -1){
            printf("Registro inexistente.\n");
            return 0;
        }
            
        // Iterando todos os registros encontrados
        while(posIndex < hi->nroRegistros && strcmp(valor,(*indices)[posIndex].chaveBusca) == 0 ){

            gvl_seek(bin, (*indices)[posIndex].byteOffset, SEEK_SET);
            aux = gvl_carregarRegistro(bin);
    ...
    int gvl_removerRegistros2(Arquivo* bin, Header* h, Index** indices, HeaderIndex* hi, char* valor){
        ...
        Registro aux;
        int qtdRegistros     = 0, posIndex, firstPos;
        long posicaoRegistro = 0;

        // Buscando o valor no vetor de indices
        firstPos = posIndex = gvl_buscarIndice(hi, indices, valor);
        ...
        while(posIndex < hi->nroRegistros && strcmp(valor,(*indices)[posIndex].chaveBusca) == 0 ){
            ...
            // Sobreescreve o registro com lixo
            Registro r;
            memset(&r, GVL_LIXO_MEM, sizeof(Registro));
            r.removido = '*';
            r.encadeamentoLista = -1; // TODO
            ...
            // Insere o registro na lista de removidos
            gvl_inserirNovoRemovido(bin, h, &r, (*indices)[posIndex].byteOffset, 1);
        ...
        // Precisa shiftar todos os elementos que foram removidos
        for(int i = 0; (posIndex+i) < hi->nroRegistros; i++){
            (*indices)[firstPos + i] = (*indices)[posIndex + i];
        }
        hi->nroRegistros -= qtdRegistros;

        return qtdRegistros;
    }

    int gvl_inserirNovoRegistro2(Arquivo* bin, Header* h, Index** indices, HeaderIndex* hi, Registro* r){
        if(bin == NULL || bin->fp == NULL || indices == NULL || hi == NULL || r == NULL || h == NULL)
            return 1;
        ...
        // Por fim, se o registro não tiver nome nulo insere ele no indice
        if(r->nomeServidor != NULL){
            Index i;
            memset(&i, GVL_LIXO_MEM, sizeof(Index));
            strcpy(i.chaveBusca, r->nomeServidor);
            i.byteOffset = posicaoInsercao;
            gvl_inserirIndice(indices, hi, i);
        }
 ...
 */
    int gvl_criarBTree(Arquivo* bin, Header* h, Arquivo* binBT){
        ...
        // Criando o cabeçalho da Árvore B e escrevendo
        HeaderArvoreB hbt = gvl_criarCabecalhoArvoreB('0');
        gvl_escreverCabecalhoArvoreB(binBT, &hbt);

        // Percorrendo os itens e inserindo na Arvore B
        gvl_seek(bin, GVL_TAM_PAGINA, SEEK_SET);
        do{ 
            byteOffset  = bin->posicaoPtr;
            reg         = gvl_carregarRegistro(bin);
            ...
            if(reg.removido == '*' || reg.idServidor == -1){
                gvl_destruirRegistro(&reg);
                continue;
            }
            ...
            // Inserindo a chave na árvore B
            if(gvl_inserirChaveArvoreB(binBT, &hbt, reg.idServidor, byteOffset)){
                printf("Deu algum ruim...\n");
            }
    ...
    int gvl_buscarRegistros3(Arquivo* bin, Header* h, Arquivo* binBT, HeaderArvoreB* hbt, int valor){
        if(bin == NULL || bin->fp == NULL || binBT == NULL || hbt == NULL)
            return 0;
        ...
        // Buscando o valor na árvore B
        qtdNiveis = gvl_buscarChaveArvoreB(binBT, hbt, valor, &byteOffset);
        ...
        gvl_seek(bin, byteOffset, SEEK_SET);
        aux = gvl_carregarRegistro(bin);

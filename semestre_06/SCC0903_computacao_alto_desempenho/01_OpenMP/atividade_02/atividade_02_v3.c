...
// Struct para guardar as amostras e suas métricas finais
...
int main(int argc, char *argv[]){
    ...
    for(int i = 0; i < N_SAMPLES; i++){
        ...
        samples[i].avg = samples[i].avg_harmonic = samples[i].coef_var = -1.0;
        samples[i].medium = samples[i].mode = samples[i].std = samples[i].var = -1.0;
    }
    ...
    #pragma omp parallel num_threads(T) shared(samples, N_SAMPLES)
    {   
